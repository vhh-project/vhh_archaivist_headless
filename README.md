...
### Containers
The application is comprised of two containers. These containers can directly communicate with eachother through an internal network automacally established by docker-compose.
...
This launches both the baseline vespa app as well as the word2word translation container. Once both container's health checks return status 'healthy' the service is ready to go.
...
Default port for vespa applications: **8080** (can be configured in *docker-compose.yml*)
...
```json
{
    ...
    "yql": "select * from sources * where body contains 'this is a demo query';",
    "hits": 5,
    "offset": 0,
